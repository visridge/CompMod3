# CompMod3 - GitHub Copilot Instructions

## Project Overview
**CompMod3** is a competitive gameplay modification for **Chivalry: Medieval Warfare**. It extends the base game (AOC folder) with balance changes, netcode improvements, and quality-of-life features for tournament play.
...
│   └── classes/            # Parent classes that CompMod3 extends
└── CompMod3/                # Our mod - extends/overrides AOC
    ├── Classes/            # UnrealScript class files (.uc)
    └── Include/            # Reusable code includes (.uci)
```

### Parent/Child Relationship
- **CompMod3 classes EXTEND AOC classes** using UnrealScript inheritance
- Example: `CompMod3Pawn extends AOCPawn`
...
- `.ini` - Configuration files (DefaultCompMod3.ini)
...
`include(CompMod3/Include/CompMod3Pawn.uci)
...
- **CompMod3Customization** - Custom character/helmet/armor system
- **Uses vanilla config system** - Saves to `Customization.ini`
- **CompMod3View_Frontend_Customization** - UI hooks for saving choices

## Development Workflow

### Making Changes
1. **Find parent class in AOC/** - Understand vanilla behavior
2. **Create/modify child class in CompMod3/** - Override specific functions
...
**Adding a new weapon:**
```unrealscript
class CompMod3Weapon_NewWeapon extends CompMod3MeleeWeapon;

// Weapon inherits all CompMod3MeleeWeapon fixes:
// - Parry buffer system
// - Stamina validation
// - Recovery parrying
...
```unrealscript
// Changes go in CompMod3Pawn.uci (shared across all pawns)
...
### CompMod3/Classes/
- `CompMod3MeleeWeapon.uc` - Base melee weapon (all weapons extend this)
- `CompMod3Pawn.uc` - Base pawn class (all pawns extend this)
- `CompMod3Weapon_*.uc` - Individual weapon implementations
- `CompMod3CharacterInfo_*.uc` - Character customization data
- `CompMod3Customization.uc` - Customization system override

### CompMod3/Include/
- `CompMod3Pawn.uci` - Shared pawn behavior (stats tracking, flinch logic)
- `CompMod3PlayerController.uci` - Shared controller behavior
- `CompMod3Game.uci` - Shared game mode behavior
...
- **CompMod3 changelog** - CHANGELOG.md in root for feature history
...
- **Follow naming conventions** - CompMod3 prefix for all custom classes
