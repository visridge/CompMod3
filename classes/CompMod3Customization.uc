class CompMod3Customization extends AOCCustomization
    config(Customization);

// Always treat all helmets as owned for CompMod3
...
// Always treat all weapon skins as owned for CompMod3
static function bool AreWeaponSkinsOwnedBy(int WeaponDrops[EWeaponType], PlayerReplicationInfo PRI)
{
	return true;
}

// Override microtransaction visibility locally for CompMod3 so the customization
...
defaultproperties
{
    CustomizationContentClassString="CompMod3.CompMod3CustomizationContent"
}

// Mod-local override: allow players on CompMod3 servers to select any helmet
// or weapon skin regardless of microtransaction ownership. We keep the
// remaining validation (characters, emblems, tabards, colors) intact.
...
{
	// CompMod3: Always accept customization to prevent "saving with locked items" warnings.
	return true;
}

// CompMod3: Override ownership checks for all cosmetic types
...
static function bool IsCharacterOwnedBy(int CharacterID, int FactionID, int ClassID, PlayerReplicationInfo PRI)
{
    ...
    // Character IDs from CompMod3CustomizationContent:
...
// CompMod3: Override LocalGetCustomizationChoices to use our unlocked logic and read from vanilla config
	...
	optional EWeaponType TertiaryWeaponType = EWEP_MAX)
{
    ...
    // CompMod3: Use local IsTabardOwnedBy (always true)
...
// CompMod3: Override to skip ownership checks and read from vanilla config
static function bool LocalGetSelectedWeaponDrops(int Faction, int PlayerClass, out int WeaponSkinArray[EWeaponType.EWEP_MAX], AOCPRI PRI)
{
    ...
    // CompMod3: Use AOCCustomization config
	...
	for(WeaponType = 0; WeaponType < EWEP_MAX; ++WeaponType)
	{
        ...
        // CompMod3: Ownership checks removed
	}

	return true;
}

// CompMod3: Override to write to AOCCustomization config
